...
### Workload attestation: spiffe-ex

Use [spiffe-ex](https://github.com/QuentinMallet/spiffe-ex) to obtain SPIFFE SVIDs for this workload via the local SPIRE agent. This is the mandatory identity mechanism — do not use static credentials or API tokens when a SPIFFE SVID is available.

Rules:
- Start `Spiffe.WorkloadAPI` in your supervision tree before any service that needs secrets or mTLS
- Fetch SVIDs via `Spiffe.WorkloadAPI.fetch_svid/0`; never cache SVIDs manually — the library handles rotation
- Use the SVID's JWT token (audience = `openbao`) when authenticating to OpenBao through secretsManager-ex

```elixir
# In Application.start/2 supervision tree
children = [
  Spiffe.WorkloadAPI,
  MyApp.Secrets,   # starts after SVID is available
  ...
]
```

### Secret management: secretsManager-ex

Use [secretsManager-ex](https://github.com/ForgottenBeast/secretsManager-ex) to fetch secrets from OpenBao, authenticated via the SPIFFE JWT SVID obtained from spiffe-ex.

Rules:
- Configure the OpenBao address and mount in `config/runtime.exs` under `:secrets_manager`; never hardcode vault addresses in application code
- Call `SecretsManager.get_secret/1` (or equivalent) to fetch secrets at runtime — do not read secrets at compile time
- Secrets must not be logged; wrap any debug output that might contain secret values with a filter
- If the SVID expires and re-authentication fails, the supervisor should restart the secrets process — do not swallow auth errors

```elixir
# config/runtime.exs
config :secrets_manager,
  openbao_addr: System.fetch_env!("OPENBAO_ADDR"),
  mount: "kv"
```

```elixir
# Fetching a secret
{:ok, value} = SecretsManager.get_secret("my-app/db-password")
```
...
defp deps do
  [
    ...
    # Identity & secrets
    {:spiffe, github: "QuentinMallet/spiffe-ex"},
    {:secrets_manager, github: "ForgottenBeast/secretsManager-ex"},
...
- spiffe-ex: https://github.com/QuentinMallet/spiffe-ex
- secretsManager-ex: https://github.com/ForgottenBeast/secretsManager-ex
